...
with tab_thinking:
    ...
    st.markdown(
        "What the agent *thinks* on every candle. Each pipeline step emits "
        "a Nogran PA-style narrative. When one stage contradicts another, "
        "the agent **changes its mind** (revision). See the full reasoning "
        "pipeline, from the feature engine to the decision scorer."
    )
    ...
    if not streams:
        ...
        for s in filtered[:limit]:
            ...
            if s.get("has_veto"):
                badges.append("⛔ veto")
            if n_rev > 0:
                badges.append(f"🔄 {n_rev} revision(s)")
...
with tab_backtest:
    ...
    st.markdown(
        "Backtests are generated by `python scripts/backtest.py`. "
        "Each run creates `logs/backtest/<run_id>/` with `summary.json`, `trades.jsonl`, `equity.csv`."
    )
